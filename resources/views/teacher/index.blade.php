    ...
    <div class="page-titles">
        <div class="d-flex align-items-center">
            ...
            <div class="custom-breadcrumb ml-auto">
                ...
                <a href="javascript:void(0)" class="breadcrumb">{{ session('Data.company_nature') == 'B' ? 'Members' : 'Students' }}</a>
            </div>
        </div>
    </div>
                            ...
                            <div class="col s12 l12">
                                ...
                                <div class="right">
                                    <form action="{{ route('data.index') }}">
                                        <div class="input-field inline">
                                            <select class="form-select inline" name="group_id" onchange="this.form.submit()">
                                                <option value="">All</option>
                                                @foreach ($Groups as $Group)
                                                <option value="{{ $Group->id }}" {{ $Group->id == $Current_Code ? 'selected' : '' }}>{{ $Group->name }}</option>
                                                @endforeach
                                            </select>
                        ...
                        <table id="demo-foo-addrow2" class="table m-b-0 toggle-arrow-tiny" data-page-size="10">
                            <thead>
                                <tr>
                                    ...
                                    <th data-hide="phone">{{ session('Data.company_nature') == 'B' ? '' : 'Parent' }}
                                        Primary Number</th>
                                    <th data-hide="{{ session('Data.company_nature') == 'B' ? 'phone' : 'all' }}">
                                ...
                                <div class="d-flex">
                                    <div class="mr-auto">
                                        <div class="form-group">
                                            <a href="{{ route('data.create') }}" class="btn btn-small"><i class="icon wb-plus waves-effect waves-light" aria-hidden="true"></i>Add New
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('data.edit', ['data' => $MobileData->id]) }}" type="button" class="btn btn-small blue m-5 left waves-effect waves-light"><i class="material-icons">edit</i></a>
                                        <form method="POST" action="{{ route('data.destroy', ['data' => $MobileData->id]) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" onclick="return confirm('Are you sure?')" class="btn btn-small red m-5 left waves-effect waves-light"><i class="material-icons">delete_sweep</i></button>
                                        </form>
                                    </td>
                                </tr>
                                @endforeach
